...
# --- QSDR combined-fib variant ---
# For population/combined QSDR fib files where src/fib are already generated and
# there is no per-subject nqa.nii.gz to derive a QA percentile from. fa_thresh_pct
# is used directly as a fraction of the [0,1] normalized QA (e.g. 10 -> 0.1),
# instead of being looked up from a histogram.
def precompute_tractography_qsdr(name, input_dir: Path, biggus_diskus: Path, seed_count=2000000, fa_thresh_pct=10, step_size=0.01, smoothing=0.01, min_length=0.5, max_length=200, turn_angle=45, name_tag=""):
    work_dir = biggus_diskus / "filtered_tracking" / f"tracking{name}dsi_studio{name_tag}-work"
    bash_stub_dir = work_dir / 'bash_stub'
    for x in [work_dir, bash_stub_dir]:
        try:
            x.mkdir(parents=True, exist_ok=False)
    ...
    fib_file = input_dir / f"{name}.template.mean.fib.gz"
    if not fib_file.exists():
        raise FileNotFoundError(f"Missing fib file for {name}: {fib_file}")

    fa_threshold = fa_thresh_pct / 100

    full_trk_file = work_dir / f"{name}.template.mean.fib.gz.2000K.tt.gz"

    if not full_trk_file.exists():
        cmd = (
            f"{DSI_STUDIO_BIN} --action=trk --source={fib_file} "+
            f"--output={full_trk_file} --fiber_count={seed_count} "+
            f"--fa_threshold={fa_threshold} --step_size={step_size} --threshold_index=qa "+
            f"--smoothing={smoothing} --min_length={min_length:.1f} --thread_count=16 "+
            f"--method=0 --turning_angle={turn_angle} --max_length={max_length:.1f}"
        )
        return make_cluster_command(cmd,bash_stub_dir,f"{name}_tracking")
    else:
        print(f"{full_trk_file} already exists.")
        return None
...
# QSDR combined-fib variant of run_both_sides
def run_both_sides_qsdr(name, roi_tuple, input_dir: Path, biggus_diskus: Path, dry_run=False, run_both_sides_override=False, label_type="RCCF", name_tag=""):
    offset = 1000
    if len(roi_tuple) == 2 and abs(roi_tuple[0] - roi_tuple[1]) == offset:
        # contralateral pair (example 47,1047) is identical to its mirror. No need to run the other side
        run_both_sides_override = True
    roi_tuple_otherside = tuple(roi + offset if roi < offset else roi - offset for roi in roi_tuple)
    cmds = []
    cmds.append(setup_pipeline_qsdr(name, roi_tuple, input_dir, biggus_diskus, label_type=label_type, dry_run=dry_run, name_tag=name_tag))
    if run_both_sides_override:
        return cmds
    cmds.append(setup_pipeline_qsdr(name, roi_tuple_otherside, input_dir, biggus_diskus, label_type=label_type, dry_run=dry_run, name_tag=name_tag))
    return cmds
...
# QSDR combined-fib variant of setup_pipeline.
# Fib/labels already live in input_dir (no connectome_cache, no per-runno prepull).
# Skips the nhdr copy/sed block since there is no per-name template .nhdr to copy from;
# add it back if SAMBA-ready nhdr outputs are needed later.
def setup_pipeline_qsdr(name, roi_tuple, input_dir: Path, biggus_diskus: Path, label_type="RCCF", dry_run=False, name_tag=""):
    work_dir = biggus_diskus / "filtered_tracking" / f"tracking{name}dsi_studio{name_tag}-work"
    results_dir = biggus_diskus / "filtered_tracking" / f"tracking{name}dsi_studio{name_tag}-results"
    bash_stub_dir = work_dir / 'bash_stub'
    for x in [work_dir, bash_stub_dir, results_dir]:
        try:
            x.mkdir(parents=True, exist_ok=False)
        except FileExistsError:
            print(f"{x} already exists")
    ...
    fib_file = input_dir / f"{name}.template.mean.fib.gz"
    label_file = input_dir / "labels" / label_type / f"{name}_{label_type}_labels.nii.gz"
    full_trk_file = work_dir / f"{name}.template.mean.fib.gz.2000K.tt.gz"
    if not fib_file.exists() or not label_file.exists() or not full_trk_file.exists():
    ...
    for i in range(len(roi_tuple)):
        ...
        if i == 0:
            ...
            roi_string_for_names=f"{roi_string_for_names}_{roi}"
        roi_string = f"{label_file}:{roi}"
        out_file = work_dir / f"{name}_{roi_string_for_names}.tt.gz"
        if i == len(roi_tuple)-1:
            out_file = results_dir / f"{name}_{roi_string_for_names}.tt.gz"
        cmd = f"{DSI_STUDIO_BIN} --action=ana --source={fib_file} --tract={tract_file} --roi={roi_string} --output={out_file}"
        cmds.append(cmd) if not out_file.exists() else cmds.append(f"#{cmd}")
    ...
    return make_cluster_command(cmds,bash_stub_dir,f"{name}_filter_and_nrrdify")
...
def main():
    ...
    # QSDR combined-fib mode: src/fib already generated, just run tracking + filtering
    parser.add_argument("--name_list", nargs="*", type=str, default=None,
                         help="QSDR combined template names, e.g. 15mo-WT-M 15mo-zQ175DN-M. "
                              "Passing this switches to the QSDR pipeline: expects "
                              "<input_dir>/<name>.template.mean.fib.gz and "
                              "<input_dir>/labels/<label_type>/<name>_<label_type>_labels.nii.gz")
    parser.add_argument("--input_dir", type=Path, default=None,
                         help="Directory holding the QSDR combined fib files and labels/ subdir. Required with --name_list.")
    ...
    if args.name_list:
        # --- QSDR combined-fib mode ---
        if not args.input_dir:
            raise ValueError("--input_dir is required when using --name_list")
        if not args.roi_tuple_list:
            raise ValueError("--roi_tuple_list is required")

        print(args.name_list)

        cmds = []
        for name in args.name_list:
            cmd = precompute_tractography_qsdr(
                name, args.input_dir, args.biggus_diskus,
                seed_count=args.seed_count, fa_thresh_pct=args.fa_thresh_pct,
                step_size=args.step_size, smoothing=args.smoothing,
                min_length=args.min_length, max_length=args.max_length,
                turn_angle=args.turn_angle, name_tag=args.name_tag,
            )
            print(cmd)
            cmds.append(cmd)
        if cmds:
        ...
        for name in args.name_list:
            for roi_tuple in args.roi_tuple_list:
                new_cmds = run_both_sides_qsdr(
                    name, roi_tuple, args.input_dir, args.biggus_diskus,
                    args.dry_run, run_both_sides_override=args.one_side_only,
                    label_type=args.label_type, name_tag=args.name_tag,
                )
                cmds.extend(new_cmds)
        cluster_run_cmds(cmds, args)
        return

    # --- original runno-based mode ---
